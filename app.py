...
if st.checkbox("Show Rolling 1-Year Alpha Summary and Distribution"):

    rolling_window = 12 # 5 years
    alpha_values = []
    alpha_dates = []
    ...
    if alpha_series.empty:
        st.warning("Not enough data to calculate rolling 1-year alpha.")
    else:
        # Summary stats
        percent_positive = (alpha_series > 0).mean()
        average_alpha = alpha_series.mean()

        st.markdown(f"- **Percent of 1-Year Windows with Positive Alpha**: **{percent_positive:.1%}**")
        st.markdown(f"- **Average Annualized Alpha (1-Year Windows)**: **{average_alpha:.2%}**")

        # Histogram
        fig, ax = plt.subplots()
        alpha_series.hist(bins=30, edgecolor='black', ax=ax)
        ax.set_title("Distribution of 1-Year Rolling Alpha")
        ax.set_xlabel("Annualized Alpha")
        ax.set_ylabel("Frequency")
        st.pyplot(fig)


if st.checkbox("Show Rolling 1-Year Sharpe Comparison"):
    ...
    st.markdown("### 📈 Rolling 1-Year Sharpe Ratio Comparison")
    st.markdown(f"- **% of 1-Year Windows Where CrestCast™ > Benchmark**: **{percent_better_sharpe:.1%}**")
